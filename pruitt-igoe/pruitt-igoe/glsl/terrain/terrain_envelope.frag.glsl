...
{ 
	vec3 lightPosition = vec3(0.0, 300.0, 1024.0);
	...
	float specular = pow(saturate(dot(refl, lightDirection)), 2.0) * (1.0 - steepness) * .3 * diffuse;
	...
	vec3 shadow = mix(vec3(.1, .35, .9), vec3(1.0), 1.0 - shadows(point, lightPosition));

	vec3 outColor = terrainColor * diffuse * 30.0 + amb * specular + amb * ambient;

	return mix(outColor, amb, saturate(-.175 + t / 2000.0)) * shadow;
}