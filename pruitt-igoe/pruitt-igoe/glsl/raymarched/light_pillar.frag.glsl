...
float noise( in vec3 x )
{
	...
	vec2 rg = texture2D( RandomTexture, (uv + 0.5) * .005).yx;
...
{
	float f;
	vec3 q = p                          - vec3(0.0,1.0,0.0)*Time * 16.0;
    f  = 0.50000*noise( q ); q = q*2.02 - vec3(0.0,1.0,0.0)*Time * 32.0;
    f += 0.25000*noise( q ); q = q*2.03 - vec3(0.0,1.0,0.0)*Time * 64.0;
